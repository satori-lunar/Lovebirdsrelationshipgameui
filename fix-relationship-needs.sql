-- Step-by-step installation for relationship needs
-- Run each section one at a time if needed

-- STEP 1: Clean up (run first)
DROP FUNCTION IF EXISTS create_partner_suggestions() CASCADE;
DROP TABLE IF EXISTS public.partner_suggestions CASCADE;
DROP TABLE IF EXISTS public.relationship_needs CASCADE;

-- STEP 2: Create relationship_needs table
...
-- STEP 3: Create partner_suggestions table
...
-- STEP 5: Create RLS policies for relationship_needs
CREATE POLICY "relationship_needs_select_policy"
  ON public.relationship_needs FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "relationship_needs_insert_policy"
  ON public.relationship_needs FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "relationship_needs_update_policy"
  ON public.relationship_needs FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "relationship_needs_delete_policy"
  ON public.relationship_needs FOR DELETE
  USING (auth.uid() = user_id);

-- STEP 6: Create RLS policies for partner_suggestions
CREATE POLICY "partner_suggestions_select_policy"
  ON public.partner_suggestions FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "partner_suggestions_update_policy"
  ON public.partner_suggestions FOR UPDATE
  USING (auth.uid() = user_id);

-- STEP 7: Create indexes
CREATE INDEX idx_relationship_needs_user_id ON public.relationship_needs(user_id);
CREATE INDEX idx_relationship_needs_relationship_id ON public.relationship_needs(relationship_id);
CREATE INDEX idx_relationship_needs_is_active ON public.relationship_needs(is_active) WHERE is_active = true;
CREATE INDEX idx_partner_suggestions_user_id ON public.partner_suggestions(user_id);
CREATE INDEX idx_partner_suggestions_shown_at ON public.partner_suggestions(shown_at) WHERE dismissed_at IS NULL AND completed_at IS NULL;

-- STEP 8: Create the suggestion generation function
