-- Create relationship needs tracking system
-- Allows users to privately indicate what's missing in their relationship
-- System then provides subtle suggestions to partners

-- Table to store what users feel is missing
CREATE TABLE IF NOT EXISTS public.relationship_needs (
...
-- RLS Policies for relationship_needs (users can only see/edit their own needs)
...
CREATE INDEX IF NOT EXISTS idx_relationship_needs_relationship ON public.relationship_needs(relationship_id);
CREATE INDEX IF NOT EXISTS idx_relationship_needs_active ON public.relationship_needs(is_active) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_partner_suggestions_user ON public.partner_suggestions(user_id);
CREATE INDEX IF NOT EXISTS idx_partner_suggestions_shown ON public.partner_suggestions(shown_at) WHERE dismissed_at IS NULL AND completed_at IS NULL;
...
BEGIN
  ...
  LOOP
    ...
    -- Check if suggestion was recently shown (within last 3 days)
    IF EXISTS (
    ...
    -- Generate suggestions based on need type
