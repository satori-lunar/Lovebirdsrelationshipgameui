import { Heart, MessageCircle, Calendar, Gift, Sparkles, Camera, Bell, Users, Link2, Settings } from 'lucide-react';
...
export function Home({ userName, partnerName, onNavigate }: HomeProps) {
  const hasCompletedDailyQuestion = false; // Mock state
  const partnerHasCompleted = true; // Mock - partner's status
  const isPartnerConnected = true; // Mock - change to false to see invite state
  const weeklyNotificationsEnabled = true; // Mock state
